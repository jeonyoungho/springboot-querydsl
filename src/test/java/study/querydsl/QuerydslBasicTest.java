...
public class QuerydslBasicTest {
    ...
    @BeforeEach
    public void before() {
        // 동시성 문제에 대해 문제 없게 설계 되었고, 스프링 컨테이너가 주입해주는 em 자체가 멀티스레드에 아무 문제 없게 설계되어있다.
        // 여러 멀티스레드에서 들어와도 현재 내 트랜잭션이 어디에 걸려있는지에 따라서 트랜잭션에 바인딩되도록 분배해준다.
        // 그래서 필드로 빼도 상관 없다.
    ...
    public void startQuerydsl() throws Exception {
        ...
        QMember m1 = new QMember("m1"); // 같은 테이블을 조인해야 되는 경우에만 이렇게 alias를 선언해서 사용하면 된다.
...
//        List<Member> content = results.getResults();
    ...
    public void aggregation() {
        List<Tuple> result = queryFactory
                ...
                .from(member)
                .fetch();
        // 데이터 타입이 여러 개 일때 querydsl에서 제공하는 TUPLE타입을 사용한다.
        // 실무에선 Tuple을 잘 쓰진 않고 DTO로 뽑아서 보통 사용한다.
...
//                .where(team.name.eq("teamA"))
    ...
    public void join_on_no_relation() {
        ...
        List<Member> result = queryFactory
                ...
                // 기존 join은 from에서 leftJoin(member.team, team) 으로 한다.
                // 그러면 자동으로 id를 기반으로 on절에 넣어서 조인을 하는데 이렇게 하면 id로 조인을 안하기에 카테세안 곱(막조인)이 된다.
                .fetch();
    ...
    @Test
    public void findDtoBySetter() { // setter 활용한 방법
    ...
    @Test
    public void findDtoByFiled() { // filed에 바로 주입하는 방법
        List<MemberDto> result = queryFactory
    ...
    @Test
    public void findDtoByConstructor() { // 생성자로 주입하는 방법, 타입을 보고 들어간다
